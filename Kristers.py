...
def get_price_kristers(brand, product):
    output_file = "ksisters_results.txt"
    results = [] # Список для хранения результатов


    url = f"https://ksisters.lv/en/brand/{brand}"
    ...
    if response.status_code != 200:
        ...
        with open(output_file, "a", encoding="utf-8") as f: # открываем файл для записи и записываем туда об ошибке
    ...
    products = soup.find_all("div", class_="product-card-text")
    if not products:
    ...
    for item in products:
        name_tag = item.find("a", class_="product-name product-name-card")
        price_tag = item.find("div", class_="product-card-price")
    ...
    # Если ничего не найдено, добавляем сообщение
    if not results:
...
if __name__ == "main":
    ...
    # Запуск парсинга
    results = get_price_kristers(brand, product)
